...
function Header() {
    ...
    const productsInCart = Object.values(productCart);
  ...
  return (
    ...
    <div className="flex-none">
        ...
        <label tabIndex="0" className="btn btn-ghost btn-circle">
            ...
            <span className="badge badge-sm indicator-item">{productsInCart.length}</span>
            </div>
        </label>
        <div tabIndex="0" className="mt-3 card card-compact dropdown-content w-52 bg-base-100 shadow">
            <div className="card-body">
            <span className="font-bold text-lg">{productsInCart.length} Items</span>
            <span className="text-info">Subtotal: ${productsInCart.reduce((acc, product) => (acc + (product.price * product.amountInCart)), 0)}</span>
